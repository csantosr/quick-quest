...
function App() {
  ...
  useEffect(() => {
    setTasks(
      JSON.parse(localStorage.getItem("tasks") ?? "[]").map((t: Task) => ({
        ...t,
        due: new Date(t.due),
      })) ?? []
    );
  ...
  return (
    <TaskContext.Provider value={{ addTask, deleteTask, toggleCompletedTask, showCompleted}}>
      <div className="flex flex-row justify-between">
        <h1 className="text-3xl text-white text-center mb-8">QuickQuest</h1>
        <div>
          <input
            ...
            />
          <label
              className="inline-block pl-[0.15rem] hover:cursor-pointer text-gray-400"
